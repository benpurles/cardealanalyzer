...
// Debug version of Zyte scraper with detailed logging
class ZyteDebugScraper {
  constructor() {
  ...
  async scrapeCarListing(url) {
    try {
      console.log('🚀 Zyte Debug scraper starting for:', url);
      ...
      // Step 2: Try Zyte API first with detailed debugging
      let carData = null;
      let zyteFailed = false;
      let zyteDebugInfo = null;
      
      try {
        console.log('🔧 Attempting Zyte API call...');
        const result = await this.scrapeWithZyte(url);
        if (result.html) {
          carData = this.extractCarData(result.html, url);
          zyteDebugInfo = result.debugInfo;
        }
      } catch (zyteError) {
        console.error('❌ Zyte API failed:', zyteError.message);
        zyteFailed = true;
        zyteDebugInfo = zyteError.debugInfo || { error: zyteError.message };
      ...
      carData.zyteDebugInfo = zyteDebugInfo;
      if (zyteFailed) {
        carData.zyteNote = 'Zyte API was unavailable, data extracted from URL patterns. Check zyteDebugInfo for details.';
      }

      console.log('✅ Zyte Debug scraping successful:', carData);
      return carData;

    } catch (error) {
      console.error('❌ Zyte Debug scraping failed:', error.message);
      throw error;
    }
  }
  ...
  // Zyte API scraping with detailed debugging
  async scrapeWithZyte(url) {
    try {
      ...
      console.log('🌐 Request options:', {
        ...
        headers: {
          'Authorization': `Bearer ${this.zyteApiKey.substring(0, 8)}...`,
      ...
      return new Promise((resolve, reject) => {
        const req = https.request(options, (res) => {
          ...
          console.log('📄 Zyte response headers:', res.headers);
          ...
          res.on('end', () => {
            console.log('📄 Zyte response body length:', data.length);
            console.log('📄 Zyte response body preview:', data.substring(0, 500));
            ...
            try {
              ...
              if (!responseData || !responseData.browserHtml) {
                ...
                const debugInfo = {
                  ...
                  hasBrowserHtml: !!responseData?.browserHtml,
                  requestUrl: url
                };
              ...
              const debugInfo = {
                statusCode: res.statusCode,
                htmlLength: responseData.browserHtml.length,
                hasScreenshot: !!responseData.screenshot,
                requestUrl: url
              };
              
              resolve({
                html: responseData.browserHtml,
                debugInfo: debugInfo
              });
            } catch (error) {
              const debugInfo = {
...
// Initialize scraper
const scraper = new ZyteDebugScraper();
...
app.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    timestamp: new Date(),
    message: 'Zyte Debug Car Scraper API',
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Zyte Debug Car Scraper API running on port ${PORT}`);
  ...
  console.log(`📝 Debug mode enabled - detailed logging available`);
}); 