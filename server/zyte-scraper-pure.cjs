...
// Universal car listing scraper using Zyte API with pure Node.js
class ZytePureScraper {
  constructor() {
  ...
  async scrapeCarListing(url) {
    try {
      console.log('🚀 Zyte Pure scraper starting for:', url);
      ...
      console.log('✅ Zyte Pure scraping successful:', carData);
      return carData;

    } catch (error) {
      console.error('❌ Zyte Pure scraping failed:', error.message);
      throw error;
    }
  }
...
// Initialize scraper
const scraper = new ZytePureScraper();
...
app.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    timestamp: new Date(),
    message: 'Zyte Pure Car Scraper API',
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Zyte Pure Car Scraper API running on port ${PORT}`);
