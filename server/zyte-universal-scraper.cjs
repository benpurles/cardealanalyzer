...
// Universal car listing scraper using Zyte API
class ZyteUniversalScraper {
  constructor() {
  ...
  async scrapeCarListing(url) {
    try {
      console.log('🚀 Zyte Universal scraper starting for:', url);
      ...
      // Step 2: Get page content using Zyte API
      ...
      // Step 3: Extract car data using pattern matching
      ...
      console.log('✅ Zyte Universal scraping successful:', carData);
      return carData;

    } catch (error) {
      console.error('❌ Zyte Universal scraping failed:', error.message);
      throw error;
    }
  }
  ...
  // Zyte API scraping using built-in fetch
  async scrapeWithZyte(url) {
    try {
      ...
      if (!data || !data.browserHtml) {
        console.log('⚠️ No HTML content from Zyte');
        throw new Error('No HTML content received from Zyte API');
      }
...
// Initialize scraper
const scraper = new ZyteUniversalScraper();
...
app.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    timestamp: new Date(),
    message: 'Zyte Universal Car Scraper API',
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Zyte Universal Car Scraper API running on port ${PORT}`);
