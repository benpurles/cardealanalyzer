...
// Universal car listing scraper using Zyte API with working fallback
class ZyteWorkingScraper {
  constructor() {
  ...
  async scrapeCarListing(url) {
    try {
      console.log('🚀 Zyte Working scraper starting for:', url);
      ...
      try {
        ...
        // If Zyte fails due to authentication, use fallback but note it
        if (zyteError.message.includes('401') || zyteError.message.includes('Unauthorized')) {
          console.log('⚠️ Zyte API authentication failed, using fallback extraction');
        }
      ...
      if (zyteFailed) {
        carData.zyteNote = 'Zyte API was unavailable, data extracted from URL patterns. For full functionality, please configure a valid ZYTE_API_KEY.';
      }

      console.log('✅ Zyte Working scraping successful:', carData);
      return carData;

    } catch (error) {
      console.error('❌ Zyte Working scraping failed:', error.message);
      throw error;
    }
  }
...
// Initialize scraper
const scraper = new ZyteWorkingScraper();
...
app.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    timestamp: new Date(),
    message: 'Zyte Working Car Scraper API',
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Zyte Working Car Scraper API running on port ${PORT}`);
